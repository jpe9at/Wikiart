...
class WikiArtDataset(Dataset):
    def __init__(self, imgdir, device="cpu", transform=None, minority_classes=None):
        ...
        self.minority_classes = set(minority_classes) if minority_classes else None
    ...
    def __getitem__(self, idx):
        ...
        if self.transform and (self.minority_classes is None or imgobj.label in self.minority_classes):
...
class FocalLoss(nn.Module):
    def __init__(self, alpha=1.0, gamma=1.0, reduction='mean', max_loss = 10):
        """
        Focal Loss
        :param alpha: Weighting factor for classes, set to 1.0 if not needed.
    ...
    def forward(self, inputs, targets):
        """
        Compute focal loss.
        :param inputs: Raw logits (not softmax probabilities).
        :param targets: True class indices.
        """
        # Convert logits to probabilities
        probs = F.softmax(inputs, dim=1)
        probs = probs.gather(dim=1, index=targets.unsqueeze(1))  # Get probabilities of true class
        probs = probs.squeeze(1)  # Remove unnecessary dimension
...
class CNNWikiArt(nn.Module):
    def __init__(self, hidden_size=300, output_size=3, optimizer='Adam', learning_rate=0.001, loss_function='CEL', l1=0.0, l2=0.0, scheduler=None, param_initialisation=None, class_weights = None):
        super(CNNWikiArt, self).__init__()
        ...
        self.batchnorm1d = nn.BatchNorm1d(173056)
        self.linear1 = nn.Linear(173056, hidden_size)
        ...
        self.loss = self.get_loss(loss_function, class_weights)
    ...
    def get_loss(self, loss_function, class_weights):
        Loss_Functions = {
